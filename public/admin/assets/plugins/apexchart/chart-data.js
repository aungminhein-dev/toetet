'use strict'; $(document).ready(function () {
    if ($('#apexcharts-area').length > 0) {
        var options = { chart: { height: 350, type: "bar", toolbar: { show: false }, }, dataLabels: { enabled: false }, stroke: { curve: "smooth" }, series: [{ name: "Students", color: '#70C4CF', data: y }], xaxis: { categories: x, } }
        var chart = new ApexCharts(document.querySelector("#apexcharts-area"), options); chart.render();
    }


    if ($('#area').length > 0) {
        var sline = { chart: { height: 350, type: 'area', zoom: { enabled: false }, toolbar: { show: false, } }, dataLabels: { enabled: false }, stroke: { curve: 'smooth' }, series: [{ name: "Students", data: y_data }], grid: { row: { colors: ['#f1f2f3', 'transparent'], opacity: 0.5 }, }, xaxis: { categories: x_data } }
        var chart = new ApexCharts(document.querySelector("#area"), sline); chart.render();
    }
